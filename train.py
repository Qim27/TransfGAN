...
class allModel(tf.keras.Model):
    def __init__(self):
        super().__init__()
        self.model_res = ResCNN.Rescnn()                                #すべてのモデルを結合するために新たに定義したモデル
    ...
    def call(self,inputList):
        ...
        #モデルの各モジュールを組み合わせ，アブレーション実験の要件に対応
...
class training():
    ...
    def call(self):         #学習開始
        ...
        evaluation.test().call()        #評価関数を統合して実行
        print('training is over')
        return
        
        
        for i in range(numberEpochs):           #学習ループ
            
            allTime = endTime - nowTime + allTime
            print(f'====== epoch:{i+1}/{numberEpochs}. time spent:{(endTime-nowTime):.3f}, total time:{allTime:.3f}. ======')             #基本情報を出力
    ...
    def train_step(self,inputList,trueImage):              #各学習ステップの処理を定義
        genertedImage = self.model(inputList)

        discriminatorLoss1 = self.discriminator.train_on_batch(trueImage,tf.ones(batchSize_s,'float32'))                        #先に識別器を学習
        ...
        generatorLoss = self.adversarialModel.train_on_batch(inputList,[trueImage,trueImage,tf.ones(batchSize_s,'float32')])    #次に生成器を学習
        #結果を出力
    ...
    def adversarial(self):      #生成器学習用モデル
    ...
    def inputImage_all(self,testImang_filerPath,allNumber=allNumber):
        ...
        for i in range(10):
            ...
            inputList_light.extend(inputImage.inImage(imgPath_light))           #画像を読み込む
            inputList_dark.extend(inputImage.inImage(imgPath_dark))
            inputList_normal.extend(inputImage.inImage(imgPath_normal))

            if(len(inputList_light) > allNumber):                               #指定数に達したらループを終了
                break
            

        state = numpy.random.get_state()                #3つのデータセットを同一順序でシャッフル
        ...
        allSize = len(inputList_light)                  #学習用とテスト用に分割
...
class lossFunction_crossEntropy(tf.keras.losses.Loss):                          #交差エントロピー損失関数
